...
static void HideDoubaoWindow(UIWindow *window, NSString *reason) {
    ...
    // 第一步：强制贴边（系统识别贴边状态，关键！）
    CGRect frame = window.frame;
    frame.origin.x = [[UIScreen mainScreen] bounds].size.width - frame.size.width + 5.0;
    window.frame = frame;
    changed = YES;
    ...
    if (changed) {
        WriteLog(@"[WINDOW] 先贴边→后隐藏 Doubao PiP reason=%@", reason);
    }
}
// ====================================================================
...
- (void)invalidateIdleTimerBehaviors {
    %orig;
    WriteLog(@"[KEEPALIVE] 贴边完成，允许屏幕息屏");
}
%end
// ====================================================================

%ctor {
    WriteLog(@"[INIT] HideDoubaoPiP 贴边隐藏版 v0.0.5");
}